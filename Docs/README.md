...
Another decision - to use Ansible.
  - I am an Ansible novicer, there are a few of challenging operations I haven't yet learned to automatize with it
     + well, manual override there.
  - the official Ansible style (regarding variables, dir layout, modularity) is mostly not kept 
     + thus no upload to Ansible Galaxy (would be a shame ;) )
...
There are two dependencies kept as files and reused on the need
  - These particular fine-tuned OpenWebRx configuration files, 3 instances of OpenWebRx
  - SDRPlay API installation binary, downloaded the vendor site.
     + hard to automatize, D/L uses CAPTCHA
...
a) install a fresh Debian (netinstall).
  ...
  - write down the user and root information, you will need these later

b) on the control host, install Ansible and sshpass.
...
c) with ansible, connect to the target and install a deploy user - script 00. This time you should use your bootstrap user.
...
Where I spent most of the time:
  - not understanding the *.yaml files syntax positional (like with Python code). One extra space in the end of a line and you are lost. Install and use yamllint.
  - differences btw various Ansible versions. You loose your time b/c some nice stackexchanged syntaxes just don't work as they did once (b/c Ansible has evolved much).
  - keep off from the Ansible official manuals as long as you can - they are useful only if you understand what you are doing.
  - some most important Ansible syntaxes are not widely (like why should become_method be described in the [privilege_escalation] and not [default] section of the ansible.cfg
...
Other possible difficulties?
  ...
  - Work through the ansible idiosyncrasies.
  - Linux admin qualifications would be useful - you need to debug it if smth goes not according to the playbook.
